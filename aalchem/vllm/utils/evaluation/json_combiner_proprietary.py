...
class JsonParser(ResponseParser):
    ...
    def parse(self, content: dict) -> Tuple[Optional[str], Optional[Dict[str, Any]], bool]:
        """Parse response content.
        
        Args:
            content: Raw response content from the model in a dictionary format
            
        Returns:
            Tuple of (reasoning_trace, parsed_json, failed_parsing)
        """
        reasoning_trace = content.get('reasoning_content', None)
        parsed_json = None
        failed_parsing = False
        json_content = content
        example_id = content.get('id', None)
        content = content.get('response_content', '')
...
class JsonCombiner:
    ...
    def extract_response_data(self, response_file: str) -> Dict[str, Any]:
        ...
        choice = data.get('response', {}).get('choices', [{}])[0]
        if choice.get('message', None):
    ...
    def extract_all_results(self) -> Tuple[List[Dict[str, Any]], List[Dict[str, Any]]]:
        ...
        for response_file in self.response_files:
            ...
            reasoning_trace, parsed_json, failed_parsing = self.parser.parse(response_data)
...
def parse_jsons_proprietary(folder: str, output: str) -> List[Dict[str, Any]]:
    """
...
if __name__ == "__main__":
    ...
    parser.add_argument("responses_folder", help="Folder containing response JSON files")
    ...
    # Create parser instance based on argument
    if args.parser == "json":
        response_parser = JsonParser()
    else:
        # Fallback to JsonParser if an unknown choice is somehow passed
        response_parser = JsonParser()
    
    # Create extractor and run
    extractor = JsonCombiner(args.responses_folder, response_parser)
    results = extractor.run(args.output)
